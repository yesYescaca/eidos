"""Experiment 08: Autonomous recovery — no external recovery_probe hints."""
...
def recovery_scenario(
    ...
    warmup: int = 10,
    n_recovery: int = 15,
) -> tuple[list[float], dict | None, dict]:
    errors: list[float] = []
    stable = vectors[target]
    hypothesis = None
    surprise_meta: dict = {}
    ...
    # v1.4: no recovery_probe — agent infers target from recent context
    ...
    surprise_meta = {
        ...
        "reasoning_triggered": result.get("reasoning_triggered"),
    }
    ...
    return errors, hypothesis, surprise_meta
...
def main() -> None:
    ...
    for target in CONCEPTS:
        ...
        inferred_ok = bool(meta["inferred_recovery_label"] == target)
        autonomous = bool(meta["recovery_inference_source"] in ("recent_history", "workspace"))
        beats_ablation = bool(early_full < early_ablated and improvement >= 10.0)
        scenario_pass = bool(correct and inferred_ok and autonomous and beats_ablation)
        all_pass = all_pass and scenario_pass

        scenario_results.append({
            "target": target,
            "selected_associate": selected,
            "inferred_recovery_label": meta["inferred_recovery_label"],
            "recovery_inference_source": meta["recovery_inference_source"],
            "hypothesis_label": hyp.get("label") if hyp else None,
            "correct_concept": correct,
            "inferred_correct": inferred_ok,
            "autonomous_inference": autonomous,
    ...
    for ax, res in zip(axes, scenario_results):
        ...
        ax.set_title(
            f"{res['target']}: inferred={res['inferred_recovery_label']}, "
            f"picked={res['selected_associate']}"
        )
        ax.set_ylabel("Early recovery error")
    plt.suptitle("Exp 08: Autonomous Recovery (v1.4)")
    plt.tight_layout()
    ...
    print("=" * 50)
    print("EXPERIMENT 08: Autonomous Recovery (v1.4)")
    print("=" * 50)
    for res in scenario_results:
        print(
            f"  {res['target']}: inferred={res['inferred_recovery_label']} "
            f"({res['recovery_inference_source']}), selected={res['selected_associate']}, "
            f"correct={res['correct_concept']}, improvement={res['improvement_percent']:.1f}%, "
            f"pass={res['pass']}"
        )
    print(f"Plot saved to {out_dir / 'results.png'}")
    print(f"PASS (all 3 scenarios, zero external hints): {all_pass}")
